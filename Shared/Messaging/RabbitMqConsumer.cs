...
public sealed class RabbitMqConsumer : IMessageConsumer
{
    ...
    public RabbitMqConsumer(RabbitMqSettings settings, ILogger<RabbitMqConsumer> logger)
    {
        ...
        // prefetchCount - process 50 max at once
        ...
        CancellationToken cancellationToken = default) where T : class
    {
        ...
        consumer.Received += async (_, ea) =>
        {
            ...
            try
            {
                var message = JsonSerializer.Deserialize<T>(json)
                    ?? throw new InvalidOperationException(
                        $"Deserialised null from queue '{queue}'.");

                _logger.LogInformation(
                    "Received {MessageType} from queue '{Queue}'.",
                    typeof(T).Name, queue);
            ...
            {
                _logger.LogError(ex,
                    "Error processing {MessageType} from queue '{Queue}'. Message will be dead-lettered.",
                    typeof(T).Name, queue);
